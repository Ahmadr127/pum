    ...
    <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
        <div class="p-6 bg-white border-b border-gray-200">
            <div class="flex justify-between items-start mb-6">
                <div>
                    <h2 class="text-xl font-bold text-gray-900">{{ $pumWorkflow->name }}</h2>
                    <p class="text-sm text-gray-500 mt-1">{{ $pumWorkflow->description ?? 'Tidak ada deskripsi' }}</p>
                </div>
                <div class="flex items-center gap-2">
                    @if($pumWorkflow->is_default)
                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-yellow-100 text-yellow-800">
                            <i class="fas fa-star mr-1"></i> Default
                        </span>
                    @endif
                    @if($pumWorkflow->is_active)
            ...
            <div class="border-t border-gray-200 pt-6">
                ...
                <div class="relative">
                    ...
                    <div class="space-y-6">
                        @foreach($pumWorkflow->steps as $step)
                        <div class="relative flex items-start">
                            <!-- Step Number Circle -->
                            <div class="flex-shrink-0 flex items-center justify-center w-12 h-12 rounded-full {{ $step->is_required ? 'bg-indigo-600' : 'bg-gray-400' }} text-white font-bold text-lg z-10">
                                {{ $step->order }}
                            </div>
            ...
            <div class="mt-6 flex gap-3 border-t border-gray-200 pt-6">
                ...
                <a href="{{ route('pum-workflows.edit', $pumWorkflow) }}" class="px-4 py-2 bg-yellow-500 text-white rounded-md hover:bg-yellow-600 font-medium">
                    <i class="fas fa-edit mr-1"></i> Edit
                </a>
                @if(!$pumWorkflow->is_default)
                    <form action="{{ route('pum-workflows.set-default', $pumWorkflow) }}" method="POST" class="inline">
                        @csrf
